...
public class GameManager {
    ...
    public int getWinner(int[][] board) {
        if (!isGameFinished(board)) { // game not finished        	
            return -1;
        } else {
            //count stones
            int p1s = boardManager.getPlayerDiscNum(board, 1);
            int p2s = boardManager.getPlayerDiscNum(board, 2);
    ...
    public boolean hasAnyMoves(int[][] board, int player) {
        return getAllPossibleMoves(board,player).size() > 0;
    }

    public ArrayList<Point> getAllPossibleMoves(int[][] board, int player) {
    ...
    public ArrayList<Point> getReversePoints(int[][] board, int player, int i, int j) {
